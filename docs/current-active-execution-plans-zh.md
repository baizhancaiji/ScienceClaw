...
- 下一批最小增量：3.3 结果归一化；只触碰 `ScienceClaw/backend/mcp/tool_factory.py` 与 `ScienceClaw/backend/tests/test_mcp_result_normalization.py`，实现 `structuredContent`、`content[].text`、普通字符串和错误对象到 offload 兼容 envelope 的归一化。
- 建议验证：
