            ...
            >
                ...
                <div class="p-5 flex flex-col gap-4">
                    ...
                    <div class="grid grid-cols-1 gap-2 bg-[var(--background-gray-main)] rounded-lg p-3 border border-[var(--border-light)]">
                        ...
                        <div v-if="model.provider === 'other'" class="flex items-center justify-between text-xs border-t border-[var(--border-light)] border-dashed pt-2">
                            <span class="text-[var(--text-tertiary)] flex items-center gap-1.5 whitespace-nowrap">
                                <Box class="size-3.5" />
                                {{ t('API Mode') }}
                            </span>
                            <span class="font-medium text-[var(--text-secondary)] capitalize">
                                {{ model.api_mode || 'completions' }}
                            </span>
                        </div>
    ...
    <Dialog v-model:open="isEditOpen">
      <DialogContent class="sm:max-w-[780px] p-0 flex flex-col max-h-[85vh] bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200/60 dark:border-gray-700/40">
        ...
        <div class="px-6 py-6 flex flex-col gap-5 flex-1 overflow-y-auto min-h-0">
            ...
            <div v-if="form.provider === 'other'" class="grid gap-2">
                <label class="text-sm font-medium text-[var(--text-secondary)]">{{ t('API Mode') }}</label>
                <div class="grid grid-cols-2 gap-2">
                    <button
                        type="button"
                        class="px-3 py-2 rounded-lg text-xs font-medium border transition-all"
                        :class="form.api_mode === 'completions' ? 'bg-blue-50 border-blue-200 text-blue-700 ring-1 ring-blue-200' : 'bg-[var(--background-white-main)] border-[var(--border-main)] text-[var(--text-secondary)] hover:bg-[var(--fill-tsp-gray-main)]'"
                        @click="form.api_mode = 'completions'"
                    >
                        other + completions
                    </button>
                    <button
                        type="button"
                        class="px-3 py-2 rounded-lg text-xs font-medium border transition-all"
                        :class="form.api_mode === 'responses' ? 'bg-blue-50 border-blue-200 text-blue-700 ring-1 ring-blue-200' : 'bg-[var(--background-white-main)] border-[var(--border-main)] text-[var(--text-secondary)] hover:bg-[var(--fill-tsp-gray-main)]'"
                        @click="form.api_mode = 'responses'"
                    >
                        other + responses
                    </button>
                </div>
...
const form = reactive({
  ...
  base_url: '',
  api_key: '',
  api_mode: 'completions' as 'completions' | 'responses',
...
const openEditModal = (model: ModelConfig | null) => {
  ...
  if (model) {
    ...
    form.api_key = '';
    form.api_mode = model.api_mode || 'completions';
