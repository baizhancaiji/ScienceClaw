...
5. **`abstract` 字段（国知局条目，规定必用）**
   ...
   - **缺失时**：若某条 JSON **无** `abstract` 或为空（旧版页面 / 表格布局未解析到等），须在查新笔记中注明「该条无摘要字段」，并优先改用**国知局详情页**、可直接打开的公开专利详情页、或通过 **Tavily** 找到的可核验公开来源补全理解后再写 1.1，**不得**留空理由含糊带过。

6. **链接与著录**：国知局详情 URL 以脚本输出为准；**禁止编造**；若仅能从公布站得到公开号，可通过 **Tavily** 或其他可访问公开源补充稳定链接，但仍须打开校验。

### B. Tavily 与可访问公开来源（**降级 / 补充**）

在 **A 不可用或结果不足**时启用，优先选择中国大陆通常可访问的公开来源：

1. **Tavily 检索**：用中文关键词、技术方案核心术语、应用场景、公开号、申请人名称进行检索。
2. **中国公开来源优先**：优先选 CNIPA 相关页面、国内知识产权资讯、专利公开转载页面、期刊官网、DOI 着陆页、机构知识库等可直接访问来源。
3. **外国专利或论文**：仅在确有帮助时补充，且必须选择当前网络环境可直接访问、可核验的公开页面。
4. **关键词构造**：技术方案核心术语、应用场景与方法名称，可组合 2–3 组查询；必要时加入申请人、IPC/CPC、行业别称。
...
| 类型 | 推荐 URL 形式 | 说明 |
|------|----------------|------|
| 美国等外国专利 | 可直接访问的官方或公开专利详情页 | 仅在当前网络环境可访问且内容可核验时使用。 |
| 中国专利 | 国知局公布站详情页或其他可直接访问的稳定公开页 | 优先国知局；勿依赖易过期的检索会话 URL。 |
| 学术论文 | 出版社官方页、机构知识库页或 **`https://doi.org/10.xxxx/...`** | 优先可直接访问且能核验题名、作者、摘要的页面。 |
...
便于写进交底书：保留专利号、标题、**消化摘要后的**一两句方案概括（有 **`abstract`** 时概括须可追溯至该摘要）；**每条另起一行或表格列给出「来源 URL」**；在 1.1「检索说明」中可简要注明是否已使用 **`cnipa_epub_search.py`**、是否触发 **Tavily/公开网页补充检索**。避免大段抄袭权利要求或整段粘贴官方摘要。
