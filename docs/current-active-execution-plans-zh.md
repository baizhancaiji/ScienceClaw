...
| --- | --- | --- | --- | --- |
| 沙盒接管页扩展施工单 | 执行中。W1/W2 已完成：`VNCViewer` 已支持运行时 `viewOnly` 切换；`TakeOverView` 已升级为会话绑定双 tab 接管页，并兼容 `?sandbox=1` / `?vnc=1`。 | `docs/sandbox-takeover-construction-plan.md` | W3 `ActivityPanel`/`TakeOverView` 按 `sessionId` 的 BroadcastChannel 快照与增量同步；W4 `BrowserToolView` 新开独立标签页入口；W5 i18n 与最小测试收口。 | `cd D:\trae\ScienceClaw\ScienceClaw\frontend && npm run type-check && npm run test:run -- src/components/TakeOverView.spec.ts src/components/VNCViewer.spec.ts` |
