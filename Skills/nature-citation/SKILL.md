...
description: "为稿件文本补充严格的 Nature/CNS 引用，将长段落拆成可引用片段，只检索 Nature Portfolio、Science 系列和 Cell Press 等认可期刊，并导出可导入文献管理器的结果。适用于自动给出引用、补引用、找引用、分段引用、Nature 系列引用、CNS 及子刊、支撑文献和 EndNote/RIS/ENW/Zotero RDF 导出。"
---
...
本技能不依赖独立 Python 脚本或 MCP Server。所有检索通过以下工具链完成：

1. **`tooluniverse_search`** → 发现可用工具（如 `Crossref_search_works`）
2. **`tooluniverse_info`** → 确认参数格式
3. **`tooluniverse_run`** → 执行 Crossref/PubMed 检索
4. **`web_search`** → 补充检索 nature.com、science.org、cell.com 等出版商标题
5. **`web_crawl`** → 验证具体页面的期刊名和 DOI

**主要检索命令**：
- `tooluniverse_run("Crossref_search_works", {"query": "english keyword"})` — 获取 DOI/标题/期刊/年份
- `web_search("site:nature.com 'keyword'")` — Nature 系列定向搜索
- `PubMed_fetch_articles` 系列 — PubMed 元数据

## Long-article strategy

| Segments | Strategy |
|---|---|
| 1–10 | 一次性处理，完整的逐句支撑分析 |
| 11–25 | 按段落拆分批次处理，输出精简汇总表 |
| 26+ | 按章节拆分，每章独立检索，最后合并去重 |
