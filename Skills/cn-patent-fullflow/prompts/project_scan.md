...
## Office 文档（.doc / .docx / .pptx）：必先转换再读

**格式**：

- **`.docx` / `.pptx`** 可直接处理。
- **`.doc`** 在 Windows 下可直接走本技能 `docx_to_md.py`，脚本会调用同目录 `doc_converter.py` 先转临时 `.docx`，原文件不覆盖；依赖 **Microsoft Word + pywin32**。
- **`.ppt`** 仍建议先在 Office / WPS 中另存为 `.pptx`。

Agent **不得**因「只能舒适读取文本」而**遗漏**项目内的 Word / PPT：**必须先转为 Markdown 再纳入扫描**，不能只扫 `.md` 与源码。

1. **发现**：在扫描目录内 **`Glob` 或列举** `*.doc`、`*.docx`、`*.pptx`（含子目录）。
2. **转换（本仓库脚本）**：对每个文件执行（路径按实际替换；`${CLAUDE_SKILL_DIR}` 为技能根）：

   ```bash
   python3 ${CLAUDE_SKILL_DIR}/tools/docx_to_md.py -i "<路径>/<名>.doc" -o "<同目录或 docs>/<名>.md"
   ...
   需已 `pip install -r requirements.txt`。处理 `.doc` 时还需本机 **Microsoft Word + pywin32**。输出旁会生成 **`{md 主名}_media/`**，内为嵌入图，**以生成的 `.md` 正文与图片引用为扫描依据**。
