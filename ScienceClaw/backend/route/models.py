...
from backend.openai_compat import OPENAI_COMPAT_HEADERS, normalize_openai_compat_base_url
...
async def verify_model_connection(
    ...
    try:
        ...
        if provider == "gemini":
            ...
            logger.info(
                f"[verify_model] Using ChatOpenAI, base_url={normalized_base_url or '(default)'}, "
                f"api_mode={api_mode}"
            )
            chat = ChatOpenAI(
                model=model_name,
                ...
                default_headers=OPENAI_COMPAT_HEADERS,
...
async def create_model(body: CreateModelRequest, current_user: User = Depends(require_user)):
    ...
    logger.info(f"[create_model] provider={body.provider}, model_name={body.model_name}, "
                f"name={body.name}, base_url={body.base_url}, api_mode={body.api_mode}, has_api_key={bool(body.api_key)}")
...
async def _probe_context_window_via_api(base_url: str | None, api_key: str | None, model_name: str) -> int | None:
    ...
    if not api_key:
        return None
    import httpx
    url = normalize_openai_compat_base_url(base_url) or "https://api.openai.com/v1"
    headers = {"Authorization": f"Bearer {api_key}", **OPENAI_COMPAT_HEADERS}
    try:
...
async def update_model(model_id: str, body: UpdateModelRequest, current_user: User = Depends(require_user)):
    ...
    merged_api_mode = update_data.get("api_mode", existing.get("api_mode", "completions"))
    
    if any(k in update_data for k in ["base_url", "api_key", "model_name", "api_mode"]):
        await verify_model_connection(
            merged_provider,
            merged_base_url,
            merged_api_key,
