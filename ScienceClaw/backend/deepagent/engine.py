...
def _resolve_use_responses_api(config: Optional[Dict[str, Any]]) -> bool:
    """Resolve whether ChatOpenAI should use the Responses API."""
    api_mode = str((config or {}).get("api_mode") or "completions").strip().lower()
...
def get_llm_model(
    ...
    if config:
        ...
        model = _SafeChatOpenAI(
            model=model_name,
            base_url=normalize_openai_compat_base_url(
                config.get("base_url") or settings.model_ds_base_url
            ),
            api_key=config.get("api_key") or settings.model_ds_api_key,
            default_headers=OPENAI_COMPAT_HEADERS,
            use_responses_api=_resolve_use_responses_api(config),
            max_tokens=effective_max_tokens,
            max_retries=3,
            request_timeout=120,
    ...
    model = _SafeChatOpenAI(
        ...
        base_url=normalize_openai_compat_base_url(settings.model_ds_base_url),
