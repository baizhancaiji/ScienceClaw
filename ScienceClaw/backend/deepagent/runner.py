    ...
    try:
        ...
        try:
            ...
            if "No generations found in stream" in str(ve):
                logger.warning("[DeepAgent] LLM returned empty stream, continuing with partial results")
            else:
                raise
        ...
        if "context length" in err_lower or "context_length_exceeded" in err_lower or "maximum context" in err_lower:
            ...
            user_msg = "Token 使用已达到限额，请稍后重试。(Token usage limit reached, please retry later.)"
