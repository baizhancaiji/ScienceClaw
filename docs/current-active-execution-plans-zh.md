...
- 状态：执行中；第 4 批 / 4.2 MCP 工具卡片与详情抽屉已按最小增量完成并进入本地验证收口；0.1 路由骨架已按 MCP 鉴权原则完成复审收敛，并将 `/mcp` router 调整为默认 `require_user` 保护；本次复审已追加枚举所有 `/api/v1/mcp/*` 路由的默认鉴权结构护栏、未认证请求行为护栏，并固定 route smoke 的认证模式以降低本地/CI 环境漂移风险。
